...
function Semesters() {
  ...
  return (
    <>
    {isOpen && <SemesterSubmit setIsOpen={setIsOpen} updateRows={setUpdate}/>}
      ...
      <table>
        ...
        <tbody>
          ...
          semesters.result.map((semester: any) => (
            <SemesterRow key={semester.id} semester={semester} setUpdate={setUpdate} />
          ))}
        </tbody>
      </table>
    </div>
    {Math.ceil(semesters.totallItems / 20) !== 1 && (
        <div className="pagiMagi">
          <ReactPaginate
            ...
            pageCount={
              semesters.totallItems ? Math.ceil(semesters.totallItems / 20) : 0
            }
            previousLabel="<"
